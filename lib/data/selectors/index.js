...
const data = createSelectorTree({
  ...
  views: {
    ...
    contractAllocationInfo: createLeaf(
      ...
      .map(({
        ...
        compilationId,
        compiler: sources[compilationId].byId[scopes[compilationId][id].sourceId]
          .compiler
      }))
    ),
        ...
        ...Object.entries(scopes).map(([compilationId, nodes]) => ({
          [compilationId]: Object.assign({},
            ...userDefinedTypes.map(
              ({
                compilationId: compilationIdForType,
                id
              }) =>
    ...
    mappingKeys: createLeaf(
      ...
      .concat(
        ...Object.values(
          (mappedPaths.byAddress[address] || {
            byType: {}
          }).byType
        ).map(({
          bySlotAddress
        }) => Object.values(bySlotAddress))
      )
    ...
    instances: createLeaf([evm.current.codex.instances], instances =>
      Object.assign({},
        ...Object.entries(instances).map(([address, {
          binary
        }]) => ({
  ...
  current: {
     ...
     */
    compiler: createLeaf([evm.current.context], ({
      compiler
    }) => compiler),
        ...
        _: createLeaf(
          ...
          (scopes, identifiers, thisDefinition) => {
            let variables = Object.assign({},
              ...Object.entries(identifiers).map(([identifier, {
                astId
              }]) => {
                if (astId !== undefined) {
