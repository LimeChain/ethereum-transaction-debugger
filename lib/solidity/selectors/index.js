  ...
  return {
    ...
    findOverlappingRange: createLeaf(
      ...
      ({
        compilationId,
        id
      }, functions) => (functions[compilationId] || {})[id]
    ),
    ...
    node: createLeaf(
      ...
      ({
        ast
      }, pointerAndNode) => (pointerAndNode ? pointerAndNode.node : ast)
    )
  };
}
...
let solidity = createSelectorTree({
  ...
  current: {
    ...
    sources: createLeaf(
      ...
      (sources, context) =>
      context ? (sources[context.compilationId] || {
        byId: null
      }).byId : null
    ),
    ...
    functionsByProgramCounter: createLeaf(
      ...
      (instructions, sources, functions, {
        compilationId
      }) =>
    ...
    nextMapped: createLeaf(
      ...
      (map, steps, index) =>
      steps.slice(index + 1).find(({
        pc
      }) => map[pc] && map[pc].file !== -1)
    )
  },
