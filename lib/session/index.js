...
import session from "../session/selectors";
import * as dataSagas from "../data/sagas";
import * as controllerSagas from "../controller/sagas";
...
import {
  shimBytecode
} from "@truffle/compile-solidity/legacy/shims";
...
export default class Session {
  ...
  constructor(compilations, provider, txHash) {
    ...
    let {
      store,
      sagaMiddleware
    } = configureStore(reducer, rootSaga);
    this._store = store;
    ...
    let {
      contexts,
      sources
    } = Session.normalize(compilations);
